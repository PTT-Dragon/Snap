...
- (void)initDeviceAndAddView:(UIView *)view WithSuccessBlock:(void(^)(NSString *qrcode))successBlock failBlock:(void(^)(NSError *error))failBlock {
    ...
    CGPoint beginPoint = CGPointMake((CGRectGetWidth(view.bounds) - wh) / 2.f, (CGRectGetHeight(view.bounds) - wh*1.5) / 2.f);
    [self.output setRectOfInterest:CGRectMake(beginPoint.y/screen.height, beginPoint.x/screen.width, wh/screen.height, wh/screen.width)];
