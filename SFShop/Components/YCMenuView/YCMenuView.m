...
- (CGPoint)getRefPoint{
    ...
    if (absoluteRect.origin.y + absoluteRect.size.height +  menuHeight > App_Frame_Height - menuHeight) {
...
- (void)calculateArrowAndFrame{
    ...
    // 考虑向左右展示不全的情况，需要反向展示
    if (originX + width > App_Frame_Width - 10) {
        originX = App_Frame_Width - kDefaultMargin - width;
    }else if (originX < 10) {
    ...
    /*
     如果不是根据关联视图，得算一次是否反向
     if (!_refView) {
         _isReverse = (originY + height > App_Frame_Width - kDefaultMargin)?YES:NO;
     }
     */
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.textLabel.textAlignment = _cellAlignment;
...
- (void)setCellAlignment:(NSTextAlignment)cellAlignment{
    
    if (_cellAlignment == cellAlignment) return;
