...
+ (void)post:(NSString *)url parameters:(nullable NSDictionary *)parameters success:(void(^)(_Nullable id response))success failed:(void(^)(NSError *error))failed {
    ...
    MPWeakSelf(self)
    [manager POST:url parameters:parameters headers:@{@"ZSMART_LOCALE":model ? (model.userRes.defLangCode ? model.userRes.defLangCode:@"id") : @"",@"accessToken":model ? model.accessToken: @""} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
...
+ (void)downloadPDF:(NSString *)url success:(void(^)(NSURL *fileURL))success failed:(void(^)(NSError *error))failed {
    ...
    [manager GET:url parameters:nil headers:@{@"ZSMART_LOCALE":model ? (model.userRes.defLangCode ? model.userRes.defLangCode:@"id") : @"",@"accessToken":model ? model.accessToken: @""} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        NSError *error = nil;
        NSString *path = [[NSTemporaryDirectory() stringByAppendingPathComponent:[NSString stringWithFormat:@"%@",kLocalizedString(@"ORDER_RECEIPT")]] stringByAppendingString:@".pdf"];
...
+ (void)post:(NSString *)url parametersArr:(nullable NSArray *)parametersArr success:(void(^)(_Nullable id response))success failed:(void(^)(NSError *error))failed {
    ...
    NSDictionary *jsonDic = [NSJSONSerialization JSONObjectWithData:objectData
                                                              ...
                                                              options:NSJSONReadingMutableContainers
                                                                ...
                                                                error:&parseError];
    MPWeakSelf(self)
    [manager POST:url parameters:jsonDic headers:@{@"ZSMART_LOCALE":model ? (model.userRes.defLangCode ? model.userRes.defLangCode:@"id") : @"",@"accessToken":model ? model.accessToken: @""} progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        id obj = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers|NSJSONReadingMutableLeaves|NSJSONReadingFragmentsAllowed error:nil];
        !success?:success(obj);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSData *responseData = error.userInfo[AFNetworkingOperationFailingURLResponseDataErrorKey];
...
+ (void)postImage:(NSString *)url image:(nullable UIImage *)image success:(void(^)(_Nullable id response))success failed:(void(^)(NSError *error))failed {
    ...
    UserModel *model = [FMDBManager sharedInstance].currentUser;
    [manager POST:url parameters:@{} headers:@{@"ZSMART_LOCALE":model ? (model.userRes.defLangCode?model.userRes.defLangCode:@"id") : @"",@"accessToken":model ? model.accessToken: @""} constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
